...
def main():
    ...
    src_dir = os.path.join(repo_dir, 'EcovacsNoiDiaVN')
    proj_dir = os.path.join(repo_dir, 'EcovacsNoiDiaVN.xcodeproj')
    
    if not os.path.exists(src_dir) or not os.path.exists(proj_dir):
        print('Error: Source folders EcovacsNoiDiaVN or EcovacsNoiDiaVN.xcodeproj not found!')
        sys.exit(1)
        
    print('[Packer] Archiving EcovacsNoiDiaVN and EcovacsNoiDiaVN.xcodeproj in memory...')
    buf = io.BytesIO()
    with tarfile.open(fileobj=buf, mode='w:gz') as tar:
        tar.add(src_dir, arcname='EcovacsNoiDiaVN')
        tar.add(proj_dir, arcname='EcovacsNoiDiaVN.xcodeproj')
